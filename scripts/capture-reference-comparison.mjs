...
await page.screenshot({ path: path.join(outDir, "hero-1440.png"), fullPage: false });

await page.locator("#benefits").scrollIntoViewIfNeeded();
await page.waitForTimeout(700);
await page.screenshot({ path: path.join(outDir, "benefits-band-1440.png"), fullPage: false });
...
await mobile.screenshot({ path: path.join(outDir, "mobile-hero-390.png"), fullPage: false });
await mobile.close();

const referenceUrl = pathToFileURL(path.join(root, "reference-target.png")).href;
const heroUrl = pathToFileURL(path.join(outDir, "hero-1440.png")).href;
const html = `<!doctype html>
<html lang="uk">
  <head>
    <meta charset="utf-8" />
    <title>Hero side-by-side</title>
    <style>
  ...
  <body>
    <div class="wrap">
      ...
      <figure>
        <figcaption>current hero 1440px</figcaption>
        <img src="${heroUrl}" />
      </figure>
    </div>
  </body>
...
const htmlPath = path.join(outDir, "hero-side-by-side.html");
...
await compare.screenshot({ path: path.join(outDir, "hero-side-by-side.png"), fullPage: true });
await compare.close();
